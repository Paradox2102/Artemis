...
public class SPIGyro {
	...
	public double getYawDiff(double angle) {
		return fixRange(angle - getYaw());
	}

	public double getPitchDiff(double angle) {
		return fixRange(angle - getPitch());
	}

	public double getRollDiff(double angle) {
		return fixRange(angle - getRoll());
	}
	...
	private static double fixRange(double angle) {
		if (angle < -180) {
			angle += 360;
		} else if (angle > 180) {
			angle -= 360;
